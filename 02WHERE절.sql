...
SELECT * FROM EMPLOYEES WHERE HIRE_DATE = '06/03/07'; --날짜비교도 문자열로 합니다.
...
SELECT * FROM EMPLOYEES WHERE FIRST_NAME LIKE '_a%'; -- a가 둘째자리인 데이터
...
SELECT * FROM EMPLOYEES WHERE JOB_ID = 'IT_PROG' OR SALARY >=5000; -- ITPROG이거나 SALARY 인 사람
SELECT * FROM EMPLOYEES WHERE JOB_ID = 'IT_PROG' AND SALARY >= 5000; -- ITPROG 인데 5000 이상인 사람
...
SELECT * FROM EMPLOYEES WHERE DEPARTMENT_ID NOT IN (50,60); -- 부서 아이디가 50, 60이 아닌 사람
SELECT * FROM EMPLOYEES WHERE JOB_ID NOT LIKE '%MAN%'; --직업명이 MAN가 아닌 사람

--------------------------------------------------------------
-- ORDER BY
SELECT * FROM EMPLOYEES ORDER BY SALARY; --아무것도 안적으면 ASC(오름차순) 입니다.
SELECT * FROM EMPLOYEES ORDER BY DEPARTMENT_ID DESC; --내림차순
SELECT FIRST_NAME, SALARY * 12 AS 연봉 FROM EMPLOYEES ORDER BY 연봉; --별칭을 ORDER절에서 사용할 수 있다.

--정렬을 2개 이상 컬럼으로 시킬 수 있음
--부서번호가 높은 사람들 중에서 , 급여가 높은 사람들 기준으로 정렬
SELECT * FROM EMPLOYEES ORDER BY DEPARTMENT_ID DESC, SALARY DESC; -- ORDER절은 ,로 AND 기능 하여 나열 사용가능
