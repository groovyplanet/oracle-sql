...
--시퀀스 사용방법 2개
SELECT DEPTS_SEQ.CURRVAL FROM DUAL; -- 현재 시퀀스 (NEXTVAL가 선행되어야 함)
...
--만약 시퀀스를 초기화 해야한다면?
--시퀀스의 증가값을 -음수로 만들어서 초기화인 것 처럼 쓸 수는 있습니다.
SELECT DEPTS_SEQ.CURRVAL FROM DUAL;

--1. 시퀀스의 증가를 (현재값-1)음수로 바꿈
ALTER SEQUENCE DEPTS_SEQ INCREMENT BY -188;
--2.현재 시퀀스를 전진
SELECT DEPTS_SEQ.NEXTVAL FROM DUAL;
--3. 다시 증가값을 1로 변경
ALTER SEQUENCE DEPTS_SEQ INCREMENT BY 1;
---------------------------------------------------------------
--시퀀스 응용 (나중에 테이블을 설계할 때 , 데이터가 엄청 많다면 시퀀스의 사용이 고려될 수 있다.)
--문자열 PK(년도값-일련번호)
--년도가 바뀌면 시퀀스를 초기화한다.
CREATE TABLE DEPTS2(
...
---------------------------------------------------------------
--INDEX
--INDEX는 PK, UNIQUE 에 자동으로 생성되고 , 조회를 빠르게 하는 HINT 역할을 합니다.
--INDEX 종류는 고유 인덱스 , 비고유 인덱스가 있습니다.
--UNIQUE 한 컬럼에는 UNIQUE 인덱스(고유)인덱스가 쓰입니다.
--일반 컬럼에는 비고유 인덱스를 지정할 수 있습니다.
--INDEX는 조회를 빠르게 하지만 , DML 구문이 많이 사용되는 컬럼은 오히려 성능 저하를 부를 수도 있습니다.
