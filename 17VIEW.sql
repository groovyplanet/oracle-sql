--뷰
--뷰는 제한적인 데이터를 쉽게 보기 위해서 미리 만들어논 가상테이블(가짜) 입니다.
--자주 사용되는 컬럼을 저장하면 , 관리가 용이합니다.
--뷰는 물리적으로 데이터가 저장된 것은 아니고 , 원본테이블을 기반으로 한 가상 테이블 이라고 생각하면 됩니다.
